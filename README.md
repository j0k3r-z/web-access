...
| 双通道连接 | Extension 模式（Chrome 扩展自动连接，免授权）+ Legacy 模式（回退） |
| 调试端口探测拦截 | 拦截网站 JS 对 Chrome 调试端口的探测请求，降低反爬检测风险 |
...
**v2.4.2 更新：**
- **Extension 双通道** — CDP Bridge Chrome 扩展自动连接 Proxy，无需手动授权 `chrome://inspect`；扩展未安装时自动回退 Legacy 模式
- **调试端口探测拦截** — Fetch.requestPaused 拦截页面对 Chrome 调试端口的 HTTP 探测，防止网站检测 CDP 环境
- **连接去重 + 错误恢复** — 防止并发重复连接，连接断开时自动清除端口缓存并重新发现
- **风控提醒** — 启动前向用户展示自动化操作风险提示
...
```
帮我安装这个 skill：https://github.com/j0k3r-z/web-access
```

**方式二：Plugin 安装**

```bash
claude plugin marketplace add https://github.com/j0k3r-z/web-access
...
```bash
git clone https://github.com/j0k3r-z/web-access ~/.claude/skills/web-access
```

## 前置配置（CDP 模式）

CDP 模式需要 **Node.js 22+**，连接 Chrome 有两种方式（二选一）：

**方式一：CDP Bridge 扩展（推荐）**
1. 在 Chrome 中加载 `extension/` 目录为解压扩展
2. Proxy 启动后扩展自动连接，无需手动授权

**方式二：Remote Debugging**
